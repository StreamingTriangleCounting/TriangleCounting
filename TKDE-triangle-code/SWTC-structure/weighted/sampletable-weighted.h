 ...
 class SampleTable
 {
	...
	void modify_triangle(sample_node* pos_s, sample_node* pos_d, long long last_mark, int op) // op = 1 means add, op = -1 means minus
	{
		...
		for(int i=0;i<cn.size();i++)
		{
			...
			int f2 = m2[u];
			int sum = f1*f2*op;
			(node_table->ID_to_pos(u))->local_count += sum;
	...
	void modify_illusion(sample_node* pos_s, sample_node* pos_d, long long landmark, int op)
	{
		...
		for(int i=0;i<cn.size();i++)
		{
			...
			int f2 = m2[u];
			int sum = f1*f2*op;
			(node_table->ID_to_pos(u))->vision_count += sum;
			pos_s->vision_count += sum;
			pos_d->vision_count += sum;
			illusion += sum;
		}
		cn.clear();\
		vector<unsigned int>().swap(cn);
	...
	void modify_both(sample_node* pos_s, sample_node* pos_d, long long last_mark, long long land_mark, int op)
	{
		...
		unordered_map<unsigned int, int> m1;
		unordered_map<unsigned int, int> m2;
		unordered_map<unsigned int, int> mi1;
		unordered_map<unsigned int, int> mi2;
		unsigned int s_num = pos_s->nodeID;
		unsigned int d_num = pos_d->nodeID;
		int edge_s = pos_s->first_edge;
		int edge_d = pos_d->first_edge;
		while (edge_s >= 0)
		{
			...
			if(edge_table->table[edge_s].timestamp>=land_mark)	// only count edges in current slice, as at next landmark only these edges will be active.
			{
