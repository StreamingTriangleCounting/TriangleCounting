...
struct count_result
{
 ...
 class SampleTable
 {
	...
	unsigned long long count_triangle(sample_node* pos_s, sample_node* pos_d, long last_mark)
	{
		vector<unsigned long> v1;
		vector<unsigned long> v2;
		map<unsigned long, int> m1;
		map<unsigned long, int> m2;
		...
		return weighted_count(v1, v2, m1, m2);
	}

	unsigned long long count_illusion(sample_node* pos_s, sample_node* pos_d)
	{
		vector<unsigned long> v1;
		vector<unsigned long> v2;
		map<unsigned long, int> m1;
	...
	count_result count_both(sample_node* pos_s, sample_node* pos_d, long last_mark)
	{
		...
		map<unsigned long, int> m1;
		map<unsigned long, int> m2;
		map<unsigned long, int> mi1;
		map<unsigned long, int> mi2;
		unsigned long s_num = pos_s->nodeID;
		unsigned long d_num = pos_d->nodeID;
		int edge_s = pos_s->first_edge;
		int edge_d = pos_d->first_edge;
		while (edge_s >= 0)
		{
			...
			if(mi1.find(tmp)==mi1.end())
			{	mi1[tmp] = 1;
				vi1.push_back(tmp);
			}
			else
				mi1[tmp]++; // count all the edges, valid or invalid in illusion count
		...
		count_result cr;
		cr.trcount = weighted_count(v1, v2, m1, m2);
		cr.illusion = weighted_count(vi1, vi2, mi1, mi2);
		return cr;
	}
	...
	void dismiss(unsigned long s, unsigned long d, int pos)
	{
	 ...
	 void insert(unsigned long s_num, unsigned long d_num, double p, long time, long land_mark,long last_mark)
	 {
		 ...
		 unsigned long random_value = rand();
		 string edge = s + to_string(random_value) + d;
	 	 unsigned int pos = (*hfunc[0])((unsigned char*)(edge.c_str()), edge.length())%size;
	 //	 unsigned int pos = rand()%size; // the pos is randomly selected.
		 ...
		 // if the inserted edge has already shown up and is sampled, int this version, difference presence of the same edge is viewed as different edge;
	...
	/*	 if (edge_table->table[pos].s == s_num && edge_table->table[pos].d == d_num)
		 ...
		 // added into the curresponding cross lists;
	 {
		...
		//		 cout << "pause 1" << endl;
			 ...
			 /*if (time > 20){
				 cout << "update" << endl;
				 cout << edge_table->table[pos].priority << ' ' << edge_table->table[pos].s << ' ' << edge_table->table[pos].d << endl;
				 cout << edge_table->table[pos].vice.priority << ' ' << edge_table->table[pos].vice.s << ' ' << edge_table->table[pos].vice.d << endl;
				 cout << endl;
			 }*/
