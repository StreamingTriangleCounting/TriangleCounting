...
using namespace std;
 ...
 class Simple_Sample
 {
		...
		void link_list(sample_node* pos_s, sample_node* pos_d, int pos, unsigned long s_num, unsigned long d_num)
	{
    	...
    	void dismiss(unsigned long s, unsigned long d, int pos) // isolate this edge from the list, conducted before delete an edge.
	{
	 ...
	 void insert(unsigned long s_num, unsigned long d_num, double p, long time, long land_mark,long last_mark)
	 {
	  ...
	  void update(long time)  // when the sampled edge expires, delete it and move the candidate edge one rank upper. Before this function the cross lists including this pos should be changed, and after this function the new sampled edge (valid or not) should be 
